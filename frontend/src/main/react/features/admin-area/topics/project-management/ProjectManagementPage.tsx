...
import { ArrowLeft, FileText, Plus, Folder, BarChart3,MessageSquare,ListPlus,Trash2,Edit3 } from 'lucide-react';
...
export default function ProjectManagementPage() {
  ...
  return (
   <AdminLayout>
        <div className="bg-[rgba(210,201,185,0.2)] px-10 pt-10 pb-16 border-b border-gray-200 rounded-b-xl">
          ...
          <div className="mt-6 text-center">
            ...
            <h1 className="text-2xl md:text-6xl font-bold">IT Project Management</h1>
              </div>
        ...
        <div className="px-10 pt-2 pb-10 bg-white ">
            <div className="px-1 pt-4 flex justify-end">
                <button className="flex items-center gap-2 bg-brand-sand text-white font-medium px-4 py-2 
              ...
              <div className="px-max-w-screen-xl mx-auto">
                <div className="flex flex-wrap justify-center gap-6 mt-6">
                  {stats.map((s, i) => (
                    <div
                      key={i}
                     className="bg-white border-l-4 border-t-2 border-brand-sand rounded-lg shadow p-4 min-w-[280px] flex-[1_1_300px]">
                      <p className="text-sm text-gray-500">{s.label}</p>
                    <div className="flex items-center justify-between mt-1">
                       <p className="text-2xl font-semibold text-gray-800">{s.value}</p>

                       
                          {i === 0 && <Folder size={26} className="text-brand-sand" />}
                          {i === 1 && <MessageSquare size={26} className="text-brand-sand" />}
                          {i === 2 && <BarChart3 size={26} className=" text-brand-sand" />}
                  </div>
                </div>
                              ))}
               </div>
  ...
  <div className="flex flex-wrap justify-center gap-6 mt-6">
    {catalogs.map((cat) => (
      <div
        key={cat.id}
        className="bg-white  border-t-8 border-brand-sand rounded-lg shadow p-4 min-w-[280px] flex-[1_1_300px]"
      >
        {/* Titel + Fragenanzahl */}
  ...
  <button className=  "flex-1 min-w-auto bg-brand-sand text-brand-navy px-2 py-1 rounded flex items-center justify-center gap-1 hover:bg-brand-sand/70"  onClick={() => navigate('/admin/catalogs')}>
